...
现任皇帝**莱斯卫**登基后，下令编纂记录帝国各地状况的丛书。恰逢学者汉布瓦西亚试图拓展对灵语的研究，于是学会受托撰写此部图斯克语专著，以辅助相关研究。

### 1.2 图斯克语的方言分布

根据皇家学会在德卓黑三世时期的认定，结合学会近期的研究，我们可以对图斯克语的方言分布做出如下描述：图斯克语方言大体可分为**沿海方言**与**山地方言**两类。

- **沿海方言**：作为诺瓦亚海贸易的主要通行语言之一，广泛分布于诺瓦亚海沿岸，包括黑蔑特方言在内，在黑蔑特第一王朝之后长期作为宫廷语言存在，大量的史书和官方记载、以及宗教文本均用古典黑蔑特方言进行撰写。沿海方言作为一个整体，其内部各方言之间的差异主要是在音段发音上存在地域差异，但在词语使用和表达风格上和黑蔑特方言保持了较强的一致性。
- 山地方言：主要分布于诺瓦亚海沿岸地区以外的山地地区，以八股纳洛地区的方言最具有代表性。最早的图斯克人、即古典时代的图斯克人的语言的很多特点可能仅在山地方言中留存，例如更严格的主宾配列系统和更加灵活的语序，以及更少的介词使用。

### 1.3 语言地位与使用情况

图斯克语言是随着图斯克人在诺瓦亚海进行的广泛商业活动和征服，而在诺瓦亚海沿岸广泛传播开来的，尤其是本书所主要描述的沿海图斯克语/海峡图斯克语。

在黑蔑特地区以及八股纳洛地区，也即图斯克核心地带，图斯克语（山地）是大多数人的母语，也是这片土地上长期以来被使用的语言。但是即便是图斯克文明发源地的图斯克山区，依旧有人群使用他们自己的语言。学会目前对这些语言的研究尚且还不够详细，在此处先不赘述。
...
海峡图斯克语的**音系**颇为简单，通过比较过去一些时期的文本和当前的文本，我们可以得出海峡图斯克语在传播的过程中应当出现了适度的简化。

海峡图斯克语基础的**元音**只有五个音段，其中两个 a、o 存在鼻化变体，但在部分方言中 i, e 可能也存在在鼻辅音附近的鼻化现象。这一点依照具体的方言而定，当代的海峡图斯克语在许多地区的发音实际上不存在明显的鼻化对立了。

海峡图斯克语存在大量双元音，尽管存在绝对音位上的差距，但是为了简化理解，一般视作两个元音音位之间的移动。

标准图斯克语的**辅音**共有 18 个音位，其实际发音在不同语音环境下可扩展为约 24 个音段变体。总体而言较为对称，存在较为规律的送气不送气对立，但缺乏浊塞音音位；龈颚音一列均为齿龈音在前元音 /i/ 前的条件变体，在正字法中以同一字母表示。
...
> **注**：上表共 18 个辅音音位，以拉丁正字法的最小对立单元为准。其中 s, z, j, c 四个音位在 /i/ 前分别实现为龈颚音 [ɕ, ʑ, tɕ, tɕʰ]（见下表的龈颚音列）。

下表按发音部位与方式列出各辅音音段在实际语音中的实现，括号内的拉丁字母为该音段在正字法中的对应书写。标有 † 的浊塞擦音为历时条件变体，非独立音位。
...
本节详述标准图斯克语动词的形态变化系统。图斯克语动词呈现出典型的高度黏着特征，其形态变化不仅是丰富构词的手段，更是构建句法框架的核心机制。动词形态的运作遵循严密的层级逻辑：越靠近词根的成分，越倾向于改变词汇的基本语义（派生）；越远离词根的成分，越倾向于通过 TMA等范畴将动词锚定在具体的句法语境中（屈折），更多是语法层面上的变形。
...
$$\mathbf{[P_1 否定]} + \mathbf{[P_2 语态/减价]} + \mathbf{ROOT}^{\mathbf{REP}} + \mathbf{[S_1 增价]} + \mathbf{[S_2 态/方式]} + \mathbf{[S_3 语气/时]}$$
...
| **S2** | **态/词汇体 (方式)** | **morg** | 彻底态    | 强调动作完成的彻底性与完结性。          |
...
> **术语与语体说明**：S2 槽位的态后缀属于**词汇体（Aktionsart）**，修饰动作本身的内部结构（如彻底性、速度、方向、真实性等）。这种黏着形式是**书面语与正式语体**的规范用法，口语中（尤其新派海峡图斯克语）已普遍将这些后缀重新分析并加上副词后缀 `-to`，构成独立的方式副词（如 `morgto`"彻底地"、`soito`"急速地"），置于动词之后修饰。目前两种用法共存：S2 后缀式被视为文雅、庄重的表达，而 `-to` 副词式则为日常口语的默认选择。详见 §3.3.1.1。
> 
> 此外，§4.3.2 中的独立标记词 `ho`（进行体）和 `po`（完成体）属于**语法体（Grammatical Aspect）**层，在句法层面将动作锚定在时间轴上，不受上述语体差异影响，两种语体中均可自由使用。
...
反复体是图斯克语中独特的形态手段，它不占据常规后缀槽位，而是通过**词根重叠（Reduplication）**实现的（在生成序列中记作 $\mathbf{ROOT^{REP}}$）�。它是动词内部的韵律构词手段，发生于 $\mathbf{S_1}$ 增价之前：词根先完成重叠形成反复词干，反复词干再作为整体接受后续的增价、方式和时态后缀。
...
当反复体与增价后缀（S1）共现时，重叠先于增价应用于词根，随后再叠加外层后缀：

> **例句 9**
> 
> 原文：`ia i koem kegegdein`
> 
> 词间对照：我 一 鸟=宾格 吃~REP-使动-FUT
> 
> 中译：**我将一直喂那只鸟。**
> 
> _解析：词根 `keg`（吃）→ 闭音节重叠 VC `kegeg`（反复词干）→ 加使动 S1 `-de` → `kegegde`（使一直吃/喂）→ 加未来时 S3 `-in` → `kegegdein`。注意反复体先于 S1 作用，重叠后的反复词干像普通词根一样接受后续后缀。_

##### 3.1.5.3 虚拟语气 (Subjunctive Mood)

虚拟语气标记 `-es` 属于 $S_3$ 屈折层，用于表达假设、条件或非实然状态。

1. **独立使用**：通常配合条件连词 `selib`（如果）。
...
> **历时与语体说明**：下表展示的严格主格—宾格系统系**古图斯克语**的特征，在书面语中至今作为规范保留。`-ir` 作为定指标记的功能自古图斯克语已有。现代海峡图斯克语口语中发生的变化在于格系统：`-ir` 的主格功能退化（语法关系转由语序承担），`-em/-im` 收缩为**高显著性有生受事的专用标记**，并扩展到有生受影响主语。口语的语法关系主要由语序（SVO）承担，完整配列规则见 §4.6。

图斯克语名词格标记系统很规则，几乎没有例外。下表展示其**书面规范形式**：
...
| 方位/伴随格/工具格 | -irt | i + 词干-it | -wor  | nwi + 词干-it |

> **历史与方言差异**：复数定指与格及方位/工具格在古图斯克语中曾存在独立后缀 `-wok` 和 `-wot`，这一区分在保守书面语体（如法律文书、宗教经典）及部分西部山地方言中仍有保留。在标准海峡图斯克语中，词尾辅音普遍弱化，`-k`、`-t` 均趋同于近似的 `-r`，导致此三格在口语中完全合流为 `-wor`。本书以标准海峡图斯克语为准，必要时以介词或语序消歧。

图斯克语中，定指形式用后缀 -ir, -wor 进行表示，相对的，显指形式由冠词 i, nwi 进行表示；复数显指的冠词在海峡图斯克语和书面中是必须的，但是小部分方言里省去了复数显指形式的冠词。

> **书面规范中的双重功能**：`-ir` 在书面规范中同时承载**定指**与**主格**两种语法功能——定指单数主语与主格后缀同形。这一双重功能自古图斯克语已有。现代口语中主格功能弱化，`-ir` 仅保留定指标记职能（见 §4.6.2）。凡本书例句中标 `NOM.DEF` 者，均指书面规范中的主格-定指复合标记，口语中对应的有生主语通常已脱落为裸词干。
...
在书面规范的语法体系中，名词性成分的格位系统通常包含五个语法范畴，其中主格、宾格、与格、属格构成四大核心格位，另有一个附加格（涵盖方位、伴随或工具等语义）作为补充。以图斯克语为例：主格通过形态标记显性编码句法主语，即谈话的核心主题或动作的发起主体；宾格则特指及物动词的直接宾语，承担动作的承受者角色；与格对应间接宾语，标记动作的接受方或受益者；属格表达所属关系，构成名词间的从属关联。附加格作为多功能标记，依据语境动态呈现方位（处所）、伴随（协同对象）或工具（手段载体）等外围语义角色，通常伴随介词出现，由不同的介词具体承载信息。

在新派的海峡图斯克语中，名词性成分的格位系统有进一步简化的倾向，非定指形式的后缀在口语中，常常失去形变；附加格有并入宾格的倾向。本书下文仍以书面规范为准。口语系统的描述见 §4.6。
...
图斯克语的指称系统为**三分结构**——定指、显指、非显指，这一区分继承自原始拉埃拉德语，在古图斯克语中已有完整体现。
...
定指形式（`-ir`/`-wor`）在书面规范的格系统中与主格后缀同形，可编码主语（详见 §3.2.1.1）。这一双重功能在主格以外的斜格中并不存在——属格、与格、方位/工具格中，定指与格后缀独立运作（如 `-il` 定指属格 vs `i + 词干 + -al` 显指属格）。定指后缀形态受词干尾音制约，当名词词干以元音结尾时，需通过连音规则与词干元音衔接。

显指冠词 `i`/`nwi` 为独立词，不黏附于词干。冠词 `nwori` 在口语中常简化为 `nwi`，其选择受语域及节律规则调节。非显指形式无任何标记，裸词干直接使用。

在格标记方面，定指和显指名词均参与完整的格变化（见 §3.2.1 表格）。非显指名词在书面规范中亦可携带格后缀（如 `japem`"肉-宾格"），唯其指称显著性低于显指形式。现代口语中，非显指名词的格标记趋于脱落（见 §4.6）。

> 复数定指形变中，复数音变规则优先作用于词干，定指标记（`-ir`/`-wor`）随后依定指性选择，形态衔接受连音规则与语源音系特征约束。
> 
> **历史与方言差异**：复数定指与格及方位/工具格在古图斯克语中曾存在独立后缀 `-wok` 和 `-wot`，该区分在保守书面语体及部分西部山地方言中仍有保留。标准海峡图斯克语中词尾辅音弱化，`-k`、`-t` 趋同于 `-r`，口语中此三格合流为 `-wor`。
...
图斯克语的副词按构成方式分为**派生副词**和**词汇副词**两大类。副词在句中位于动词**之后**，与图斯克语中心语前置的原则一致。

#### 3.3.1 派生副词

派生副词通过多种异形手段从形容词词根生成。图斯克语的副词化手段多源、不规则，包括规则后缀、零派生、化石后缀及重叠式四种途径，其能产性从高到低递减。

##### 3.3.1.1 后缀 `-to`

大多数形容词通过后缀 `-to` 派生成方式副词。这是唯一能产的副词化手段。

| 形容词 | 释义 | 派生副词 | 释义 |
|--------|------|----------|------|
| siw | 小 | siwto | 小地，稍微地 |
| syerau | 好，善良 | syerauto | 好地，友善地 |
| rol | 深 | rolto | 深地 |

> **语体差异**：在口语及新派海峡图斯克语中，派生于动词态后缀（见 §3.1.4 S2）的 `-to` 副词已取代了原有的黏着后缀式。例如，书面语 `kegmorg`（吃~彻底态）在口语中通常表达为 `keg morgto`（吃 彻底地）。这一趋势被视为图斯克语从高度黏着向分析化缓慢演变的典型标志。书面语中直接使用 S2 后缀仍被视为正式、文雅，常见于法律文书、宗教经典和学术著作。

##### 3.3.1.2 零派生（封闭词类）

部分常用性质形容词不需要后缀，直接用作副词。此为零派生，属于封闭词类，不接受新成员。
...
古老的派生方式，已不再能产。保留在一批常用词上，词汇化程度高。由于古图斯克语与海峡方言间的一系列历时音变（参见 §2.5），部分形式存在不规则连音现象。

| 形容词     | 副词       | 释义     | 备注                           |
| ------- | -------- | ------ | ---------------------------- |
| lai     | lai      | 大地     | 形副同形（词干末 `i` + 后缀 `i` → `i`） |
| glat    | glaci    | 滑地，顺畅地 | `t` 在 `i` 前颚化为 `c`           |
| twaknua | twaknuai | 急地，迅速地 | 词干末 `a` + `i` → `ai`         |
| nadziu  | nadziui  | 近地     | `u` + `i` → `ui`             |

##### 3.3.1.4 重叠式（强调/反复副词）

少数形容词的副词通过词根部分重叠形成，表示强调或反复。此手段仅适用于个别词，属封闭集合。

| 形容词     | 副词         | 释义    |
| ------- | ---------- | ----- |
| twaknua | twatwaknua | 急匆匆地  |
| sisic   | (si)sisic  | 疯疯癫癫地 |
...
##### 3.3.1.6 派生副词与形容词的功能区分

| 类别 | 句法功能 | 例句 |
|------|----------|------|
| **形容词** | 作定语，位于名词之后 | `kenir lai`（那只大狗） |
| **派生副词** | 作状语，位于动词之后 | `ia keg lai`（我吃 大地） |

---

#### 3.3.2 词汇副词

词汇副词是不通过派生手段、直接作为副词使用的词。按语义分为以下几类：
...
副词**必须紧跟在它所修饰的动词之后**，位于宾语之前。这一强制性规则在图斯克语的 N-Adj 语序下具有消歧功能：形容词出现在名词后（N + Adj），副词出现在动词后（V + Adv），二者句法位置互不重叠。
...
| 例句 | 中译 |
|------|------|
| `mbo ud magei` | 他死 坏地 → 他死得很惨 |
| `ia keg aucito japem` | 我吃 安静地 肉 → 我安安静静地吃肉 |

##### 3.3.3.3 程度副词的位置

程度副词通常在所修饰的形容词之后（Adj-Adv），遵循 N-Adj 的结构逻辑：

| 结构 | 释义 |
|------|------|
| `lai soar` | 大 多 → 很大 |
| `mbo keg hau` | 他吃 少 → 他吃得少 |
...
| 例句 | 中译 |
|------|------|
| `ia leb lasu` | 我来 常常 → 我常常来 |
| `ia ar eha` | 我在 这里 → 我在这里 |
| `mbo leb sak` | 他来 那里 → 他从那里来 |

> **注**：由于图斯克语是话题优先语言，当时间/地点信息作为话题被强调时，可移至句首，形成 **话题-SOV** 结构。
...
| 类别 | 形成方式 | 示例 |
|------|----------|------|
| 派生副词 | 多源异形（`-to`、零派生、`-i`、重叠） | `siwto`、`magei`、`glaci`、`twatwaknua` |
| 词汇副词 | 直接使用 | `soar`、`hau`、`eha`、`lasu` |
...
图斯克语人称代词系统通过多层次的形态分化和词汇选择，构建起反映社会等级制度的称谓体系。从特称到谦称共五个层级，语法的复杂度与人称层级正相关——层级越高，格形态越精致（乃至完全不变格，以不变应万变），层级越低，格形态越简化或走向规整。

---

###### 3.4.1.1.2 基础代词

基础代词用于日常平等交际，复数通过后缀 `-ki` 构成。各格后缀附着后发生连缀缩合，缩合规则遵循 §2.2.1 元音连缀规则。
...
> **缩合说明**：`ia+em → iam`（ia→双元音→取 a，/ɛ/ 失时值）、`sou+em → som`（双元音在前，单元音脱落）、`mbo+em → mbom`（o+em 无对应双元音，取 o）。属格 `+al`、与格 `+ce`、方位格 `+it` 类推。

**复数**（后缀 `-ki`，复数代词的格缩合程度较低，`-ki` 作为音节屏障）：

| 功能     | iaki（我们） | souki（你们） | mboki（他们） |
| ------ | -------- | --------- | --------- |
| 主语     | iaki     | souki     | mboki     |
...
> 复数代词的与格和方位格已合并为统一后缀 `-it`，直接附着于复数词干后。两种格功能的具体释义依赖上下文或介词区分。
...
第一人称尊称 hirin（吾）和第二人称尊称 sorin（您）的词干分别为 `hiri-`、`sori-`。主格为 `hiri+ir → hirin`、`sori+ir → sorin`（依元音连缀规则）。斜格（除主格以外的各格）高度缩合——非重读的 `-ri-` 音节在辅音后缀前脱落：
...
> **同形说明**：尊称第二人称受事 `som` 与基础代词第二人称受事 `som`（sou+em）同形。语境自然消歧——尊称与基础代词不可能由同一说话者在同一语境中对同一对象使用。

**复数**：加后缀 `-ki` → **hiki**（hiri+ki）、**siki**（sori+ki），双音节。正式书信中可保留 `-wor` 形式 → hiriwor / soriwor（偏书面语）。

- mibok

第三人称尊称 mibok（怹）的词源为古复数 `mboki`（他们），经语义升格为单数尊称。词尾 `-k` 已重新分析为词干辅音，各格后缀的辅音按名词系统的交替类推（见 §3.2.1）：
...
| 与格 | mibok | 与主格同形（-k 已承载格功能） |
| 方位 | mibot | 名词 -ir : -irt = mibok : mibot |

**复数**：**miboki**（加 `-ki`），书面中保留 `-wor` 形式 → **mibokwor**。

> mibok 在图斯克-黑蔑特一带仅用于称呼神祇，在咕洛族上层中也可用于祖父辈尊长。与格与主格同形时，口语可借用咕洛语来源的位格词缀 `-met` 消歧（`mibok-met`"在怹那里"）。

---

###### 3.4.1.1.6 特称

君主自称 **Sekui**（朕），与一般人自称做区分；臣民对君主使用 **Kuisou**（陛下）。两词均属仪式性语域，完全不变格，语法关系仅由语序承载。特称的不变性本身就是神圣/权威在场的语言学标记。

**Kuisou 作宾语的迁说法**：当 Kuisou 出现在宾语位置时，书面语不能直接用不变格形式充任宾语，替代策略为属格迁说法——将陛下编码为领属者，再以一个抽象名词作为真正的句法宾语：

> **看见陛下**
>
> 原文：`ia jyom gu Kuisouil honem`
>
> 对照：我 看 过去时 陛下=属格 尊容=受事
>
> 中译：我看见了陛下。（直译：我看见了陛下的尊容。）

> **听见陛下**
>
> 原文：`ia vir gu Kuisouil zher`
>
> 对照：我 听 过去时 陛下=属格 舌/言语
>
> 中译：我听见了陛下。（直译：我听见了陛下的言语。）

> **向陛下进献**
>
> 原文：`ia kwede gu syecim Kuisoum`
>
> 对照：我 献 过去时 剑=受事 陛下=与格
>
> 中译：我向陛下进献了剑。
>
> 进献类动词属三价动词，可直接使用与格迁说法 `Kuisoum`（Kuisou+与格 `-m`），省略完整迁说结构。

Kuisou 作属格时直接加 `-il`：`Kuisouil pahelek`（陛下的宫殿）。特称无复数形式。

---

###### 3.4.1.1.7 自指谦称

谦称系统用于说话者贬抑自身，复数后缀为 `-ti`。

| 形式 | 性别 | 释义 | 主语 | 受事 | 属格 | 复数 |
|------|------|------|------|------|------|------|
| wan | 通用 | 在下 | wan | wanem | wanal | wanti |
| wak | 男 | 鄙 | wak | wakem | wakil | —（用 wanti） |
| haam | 女 | 妾 | haam | ham | hamil | hamti |
| sewank | 男 | 此身 | sewank | sewankem | — | 无 |
| sewam | 女 | 此身 | sewam | sewamem | — | 无 |

> `sewank`（男）与 `sewam`（女）含指示前缀 `se-`（近指）及身体词干 `wank`/`wam`，字面义"此身"，故无复数。谦称几乎不出现在与格和方位格位置——贬抑自身在语法上不适合充当接受者或地点参照点。
...
| 形式 | 地域 | 释义 | 格标记方式 | 复数 |
|------|------|------|-----------|------|
| omsou | 北部 | 阁下 | 半变格（宾 omsom, 属 omswal, 与 soc） | omsoki |
| lakome | 南部 | 大人 | 走名词完整变格（lakomeir, lakomem, lakomeil, …） | lakomewor |

> lakome 非代词，为与尊称功能等同的普通名词，格变化见 §3.2.1。omsou 可能与 `om-`（前缀，来源不明）+ `sou`（你）同源，与格以下罕见，口语中常直接作为不变格形式使用。

> **缺省规则**：凡上列各表未明确给出的格位或数形式，一律按普通名词的变格法（§3.2.1）类推。例如：谦称 sewank 未列出属格，需要时按名词规则构成 `sewankil`；复数按名词规则加 `-wor` 或冠词 `nwi` 构成。代词系统与名词系统并非截然分立——未被专用代词形式覆盖的空缺，由名词变格系统填补。
...
> 原句：`syecir pail ial`
> 
> 词间对照：剑=单数主格定指 父亲=单数属格非定指 第一人称属格
> 
> 中译：我父亲的剑
...
> **注**：§4.3.1 中所有例句均沿用书面规范形式（主格 `-ir` 显性标记）。在现代口语中，有生主语的主格标记已蜕变为可选的定指标记，语法关系主要由语序承担。详 §4.6。

图斯克语的体系统分为两个层级：**词汇体**（见 §3.1.4 S2 槽位）通过后缀修饰动作的内部方式，属于构词层面，书面语中为黏着后缀，口语中多转为 `-to` 副词（见 §3.3.1.1）；**语法体**通过独立标记词 `ho`（进行体）和 `po`（完成体）定位动作在时间中的展开状态，属于句法层面，两种语体通用。二者互不冲突——词汇体成分在内层（后缀或副词），语法体标记在外层。
...
图斯克语配列从严格主—宾格到显著性驱动的转变经历了三个阶段：

1. **古图斯克语阶段**（奎列奥王朝—黑蔑特第一王朝）：严格主格 `-ir` / 宾格 `-em/-im` 对立，所有论元显性标记，语序高度自由。见 §3.2.1。

2. **中古图斯克语阶段**（黑蔑特第二王朝—焦利亚王朝）：口语中语序开始锁定为 SVO。有生名词天然趋近施事诠释，无生名词天然趋近受事诠释，格标记的信息冗余逐步增加。定指标记 `-ir` 开始脱离主格功能，向独立的指称标记漂移。

3. **现代海峡图斯克语口语**（铁苏库王朝以降）：格系统简化。`-ir` 的主格功能弱化，仅保留固有定指标记职能；`-em/-im` 收缩为高显著性有生受事标记及受影响主语标记。语法关系主要由语序承担。

#### 4.6.2 显著性三分系统

现代口语中，普通名词的指称显著性通过三种形式标记。人称代词（见 §3.4.1.1）天然 [+定指]，不参与此三分，格变化走独立的缩合路径。
...
> **功能转用**：这一三分系统直接继承自古图斯克语。`-ir` 自古即兼表定指与主格——该双重功能中主格一侧的弱化是口语演变的主线（见 §4.6.1），定指标记一侧始终保持。显指标记 `i` 与非显指的裸词干同属古图斯克语的固有指称范畴。

#### 4.6.3 差异宾语标记

现代口语中，动词宾语是否带受事标记取决于其有生度与显著性层级：

| 层级 | 类别 | 受事标记 | 示例 |
|------|------|---------|------|
| **I** | 人称代词 | 强制（缩合路径） | `ia jyom gu mboem`（我看见了他） |
...
> 层级 I、II 的强制标记源于歧义消除需求：人称代词和定指有生名词既可充当施事也可充当受事，若不加受事标记可能与主语混淆。层级 IV 的无生名词天然倾向受事解读，加形态标记则为冗余。
...
>
> 有生主语 `ken` 零标记（书面中写作 `kenir`）；无生主语 `heimir` 带定指 `-ir`（因"太阳"天然定指），但这是定指标记而非主格——`heimir` 出现在宾语位时也使用同一形式，如 `ia jyom gu heimir`（我看见了太阳），语法关系主要靠 SVO 语序判定。

#### 4.6.5 受影响主语

受事标记 `-em/-im` 在口语中扩展至**有生名词**充当非自主动词主语的场景，标记主语为动作的经历者/承受者而非发起者。
...
> 中译：他遇害了 / 他死于非命。
>
> _解析：有生主语 `mbo` 附加受事标记 `-em`，暗示死亡非自然发生。对比零标记 `mbo ud gu`，仅为陈述死亡事实，不携带死因信息。_
...
> 中译：我静下来了（不由自主地）。
>
> _解析：与自主形式 `ia aucibor gu`（我使自己安静下来）形成自主—非自主对立。_

> **注**：无生主语不使用受影响主语标记——无生名词本身缺乏自主性，加受事标记不产生语义对立。
...
标准海峡图斯克语中，斜格（与格、方位格）标记与介词**叠用**而非替代——介词约束语义域，格后缀承担语法关系。例如：
...
介词与格后缀双层标记是书面规范的要求，在正式演说、公文和法律文书中遵循。

**方言简化趋势**：部分东部山地及咕洛语接触区方言中，斜格后缀在介词充足时可脱落，形成纯介词结构（如 `wi mbo`"向他"）。这一趋势在新派海峡图斯克语口语中开始渗透，但在标准语中仍被视为不规范。与格在有生间接宾语上保留黏着形式（如 `kwede gu syecim mboce`"我给了他剑"），脱落主要发生在无生名词和地点名词上。

属格 `-il` 在现代口语中仍有能产性，但名词—名词无标领属结构也在增长，尤其当领属者为非定指名词时（`syecir ken`"一只狗的剑" vs `syecir kenil`"那狗的剑"——后者因定指而更倾向带属格标记）。
